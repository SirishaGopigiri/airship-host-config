...
interested_labels_annotations = [
        ...
        "Kubelet Version", "Operating System", "hardwareclassification"
        ]
...
class KubeInventory(object):
    ...
    def __init__(self):
        ...
        if os.environ.get("TARGETCLUSTER_NAME") and os.environ.get("TARGETCLUSTER_NAMESPACE"):
            self.api_instance = kubernetes.client.CoreV1Api(
                kubernetes.config.load_incluster_config())
            namespace = os.environ.get("TARGETCLUSTER_NAMESPACE")
            name = os.environ.get("TARGETCLUSTER_NAME")+"-kubeconfig"
            secret_value = self.api_instance.read_namespaced_secret(
                        name, namespace)
            kubeconfig = (base64.b64decode(
                    secret_value.data['value'])).decode("utf-8")
            fileName = "/tmp/"+name
            with open(os.open(
            ...
            self.api_instance = kubernetes.client.CoreV1Api(
                kubernetes.config.load_kube_config(fileName))
        else:
            self.api_instance = kubernetes.client.CoreV1Api(
                kubernetes.config.load_incluster_config())
        if self.args.list:
